...
Poseidon::StreamBuffer decrypt(Poseidon::StreamBuffer ciphertext){
	...
	boost::array<unsigned char, 16> username;
	// USERNAME: 16 bytes
	DEBUG_THROW_UNLESS(ciphertext.get(&username, 16) == 16, Poseidon::Cbpp::Exception, Protocol::error_end_of_stream, Poseidon::sslit("End of stream encountered, expecting USERNAME"));
	const AUTO(user_it, g_authorized_users.find(username));
	DEBUG_THROW_UNLESS(user_it != g_authorized_users.end(), Poseidon::Cbpp::Exception, Protocol::error_authorization_failure, Poseidon::sslit("User not found"));
	boost::uint64_t temp64;
	// TIMESTAMP: 8 bytes
	DEBUG_THROW_UNLESS(ciphertext.get(&temp64, 8) == 8, Poseidon::Cbpp::Exception, Protocol::error_end_of_stream, Poseidon::sslit("End of stream encountered, expecting TIMESTAMP"));
	const boost::uint64_t timestamp = Poseidon::load_be(temp64);
	DEBUG_THROW_UNLESS(timestamp >= Poseidon::saturated_sub(utc_now, g_message_lifetime), Poseidon::Cbpp::Exception, Protocol::error_authorization_failure, Poseidon::sslit("Request expired"));
	DEBUG_THROW_UNLESS(timestamp < Poseidon::saturated_add(utc_now, g_message_lifetime), Poseidon::Cbpp::Exception, Protocol::error_authorization_failure, Poseidon::sslit("Timestamp too far in the future"));
	// KEY_CHECKSUM: 8 bytes
	boost::array<unsigned char, 8> checksum;
	DEBUG_THROW_UNLESS(ciphertext.get(checksum.data(), 8) == 8, Poseidon::Cbpp::Exception, Protocol::error_end_of_stream, Poseidon::sslit("End of stream encountered, expecting KEY_CHECKSUM"));
	Poseidon::Sha256_ostream sha256_os;
	const std::size_t ciphertext_length = ciphertext.size() - 8; // Remember to drop the DATA_CHECKSUM.
	sha256_os <<timestamp <<'#' <<user_it->second <<'#' <<ciphertext_length <<'#';
	AUTO(sha256, sha256_os.finalize());
	DEBUG_THROW_UNLESS(std::memcmp(checksum.data(), sha256.data(), 8) == 0, Poseidon::Cbpp::Exception, Protocol::error_authorization_failure, Poseidon::sslit("Incorrect key (checksum mismatch)"));
	const AUTO(aes_key, aes_key_init_192(sha256.data() + 8));
	// DATA_CHECKSUM: 8 bytes
	DEBUG_THROW_UNLESS(ciphertext.get(checksum.data(), 8) == 8, Poseidon::Cbpp::Exception, Protocol::error_end_of_stream, Poseidon::sslit("End of stream encountered, expecting DATA_CHECKSUM"));
	// DATA: ? bytes
	aes_ctr_transform(plaintext, ciphertext, aes_key);
	sha256_os <<timestamp <<'#' <<plaintext <<'#';
	sha256 = sha256_os.finalize();
	DEBUG_THROW_UNLESS(std::memcmp(checksum.data(), sha256.data(), 8) == 0, Poseidon::Cbpp::Exception, Protocol::error_bad_request, Poseidon::sslit("Request not recognized (checksum mismatch)"));
	return plaintext;
}
